...
const PostItem = ({ post }: { post: Post }) => {
  ...
  return (
    <div className="p-6 flex gap-4 hover:bg-white/2 transition">
      <Link
        to={`/${post.username === user.username ? user.username : post.username}`}
      >
        {/* <img src={post.avatar} className="w-10 h-10 rounded-full" /> */}
        <Avatar>
          <AvatarImage
            src={post.avatar === user.avatar ? user.avatar : post.avatar}
          />
          <AvatarFallback>
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center gap-2 text-sm ">
          ...
          >
            <span className="font-semibold mr-2">
              {post.name === user.name ? user.name : post.name}
            </span>
            <span className="text-white/40">
              @{post.username === user.username ? user.username : post.username}
            </span>
          </Link>
        ...
        {/* CONTENT */}
        <Link
          to={`/${post.username === user.username ? user.username : post.username}/posts/${post.id}`}
        >
